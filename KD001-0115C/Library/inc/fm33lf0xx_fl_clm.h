...
#ifndef __FM33LF0XX_FL_CLM_H
#define __FM33LF0XX_FL_CLM_H
...
{
    MODIFY_REG(CLMx->CFGR, (0xffU << 8U), (cycle << 8U));
}
...
{
    return (uint32_t)(READ_BIT(CLMx->CFGR, (0xffU << 8U)) >> 8U);
}
...
{
    return (uint32_t)(READ_BIT(CLMx->CNTR, (0xffffffU << 0U)) >> 0U);
}
