...
/** @addtogroup FM33FR0xx_FL_Driver_UART
  * @{
  */
...
FL_ErrorStatus FL_UART_DeInit(UART_Type *UARTx)
{
    ...
    if(UARTx == UART0)
    {
        ...
        /* 外设工作时钟关闭 */
        FL_CMU_DisableGroup1OperationClock(FL_CMU_GROUP1_OPCLKEN_UART0);
    }
    else if(UARTx == UART1)
    {
        /*复位UART*/
        ...
        /* 外设工作时钟关闭 */
        FL_CMU_DisableGroup1OperationClock(FL_CMU_GROUP1_OPCLKEN_UART1);
    }
    else if(UARTx == UART2)
    {
        /*复位UART*/
        ...
        FL_CMU_DisableGroup1OperationClock(FL_CMU_GROUP1_OPCLKEN_UART2);
    }
    else if(UARTx == UART3)
    {
        /*复位UART*/
        ...
        FL_CMU_DisableGroup1OperationClock(FL_CMU_GROUP1_OPCLKEN_UART3);
    }
    else if(UARTx == UART4)
    {
        /*复位UART*/
        ...
        /* 总线、工作时钟关闭 */
        FL_CMU_DisableGroup1OperationClock(FL_CMU_GROUP3_BUSCLK_UART4);
    }
    else
    {
        status = FL_FAIL;
...
FL_ErrorStatus FL_UART_Init(UART_Type *UARTx, FL_UART_InitTypeDef *initStruct)
{
    ...
    if(initStruct != NULL)
    {
        ...
        if(UARTx == UART0)
        {
            ...
            FL_CMU_EnableGroup1OperationClock(FL_CMU_GROUP1_OPCLKEN_UART0);
        }
        else if(UARTx == UART1)
        {
            FL_CMU_EnableGroup3BusClock(FL_CMU_GROUP3_BUSCLK_UART1);
            FL_CMU_EnableGroup1OperationClock(FL_CMU_GROUP1_OPCLKEN_UART1);
        }
        else if(UARTx == UART2)
        {
            FL_CMU_EnableGroup3BusClock(FL_CMU_GROUP3_BUSCLK_UART2);
						FL_CMU_EnableGroup1OperationClock(FL_CMU_GROUP1_OPCLKEN_UART2);
        }
        else if(UARTx == UART3)
        {
            FL_CMU_EnableGroup3BusClock(FL_CMU_GROUP3_BUSCLK_UART3);
						FL_CMU_EnableGroup1OperationClock(FL_CMU_GROUP1_OPCLKEN_UART3);
        }
        else if(UARTx == UART4)
        {
...
FL_ErrorStatus FL_UART_InfraRed_Init(UART_Type *UARTx, FL_UART_InfraRed_InitTypeDef *initStruct)
{
    ...
    if(initStruct != NULL)
    {
        ...
        /*红外调制频率*/
        tempTZBRG = (uint32_t)((FL_CMU_GetAPBClockFreq()) / initStruct->modulationFrequency - 1);
        /* 调制占空比 */
        if((tempTZBRG >> 4) != 0)
        {
