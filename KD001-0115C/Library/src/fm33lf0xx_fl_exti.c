...
#define IS_EXTI_CLK_SOURCE(__VALUE__)              (((__VALUE__) == FL_CMU_EXTI_CLK_SOURCE_AHBCLK)||\
                                                    ((__VALUE__) == FL_CMU_EXTI_CLK_SOURCE_RCLP))
...
FL_ErrorStatus FL_EXTI_CommonInit(FL_EXTI_CommonInitTypeDef *EXTI_CommonInitStruct)
{
    ...
    /* 使能并配置外部中断时钟源 */
    FL_CMU_EnableGroup1OperationClock(FL_CMU_GROUP1_OPCLKEN_EXTI);
...
{
    /* 关闭外部中断时钟源 */
    FL_CMU_DisableGroup1OperationClock(FL_CMU_GROUP1_OPCLKEN_EXTI);
    return FL_PASS;
}
...
{
    EXTI_CommonInitStruct->clockSource = FL_CMU_EXTI_CLK_SOURCE_RCLP;
}
