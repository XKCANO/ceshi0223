...
void RunADC_GatherProc(void)
{
	switch(ADC_struct.Step)
	{
		case 0://设置优先级，电压采集 > Lppm采集
			if((ADC_struct.BatState == 0x1) && (ADC_struct.HppmState == 0x1))
			{
				ADC_struct.HppmState = 0x2;
			}
			if((ADC_struct.HppmState == 0x2) && (ADC_struct.BatState == 0x0))
			{
				ADC_struct.HppmState = 0x1;
			}
			ADC_struct.Step++;
			break;
		case 1:
		...
		case 3:
			if(((ADC->ACQ_ISR >> 0x4U) & 0x1U) == 0x1)
			{
				...
				if(ADC_struct.HppmState == 0x1)
				{
					ADC_struct.Ppm[0] = (uint32_t)ADC->SCANACQ_DR;
				}
				ADC_struct.Step++;
			}
			break;
...
//					}
		...
		case 5:
			if(((ADC->ACQ_ISR >> 0x4U) & 0x1U) == 0x1)
			{
				if(ADC_struct.HppmState == 0x1)
				{
					ADC_struct.Ppm[1] = (uint32_t)ADC->SCANACQ_DR;
					ADC_struct.Ppm[2] = (ADC_struct.Ppm[0] + ADC_struct.Ppm[1]) / 2;
					ADC_struct.Ppm[2] = ADC_struct.Ppm[2] *( Display.Bat / 3.0);
				}
//				if(*((uint32_t *)FLASH_Count) == 0x1)
//				{
//					if(ADC_struct.Ppm[2] <= *((uint32_t *)FLASH_HunderdPpm))
//					{
//						Display.Ppm = 100.0 * ((int)ADC_struct.Ppm[2] - *((int *)FLASH_ZeroPpm)) / (*((int *)FLASH_HunderdPpm) - *((int *)FLASH_ZeroPpm));
//						Display.Ppm = Display.Ppm > 0 ?  Display.Ppm:0;
//					}
					if(ADC_struct.Ppm[2] <= 0x425)
					{
						Display.Ppm = 100.0 * ((int)ADC_struct.Ppm[2] - 0x10B) / (0x425 - 0x10B);
						Display.Ppm = Display.Ppm > 0 ?  Display.Ppm:0;
					}
//					else
//					{
//						Display.Ppm = (float)(100 * (ADC_struct.Ppm[2] - *((int *)FLASH_ZeroPpm)) - 17 *  ADC_struct.VAREF * pow((float)((ADC_struct.Ppm[2] - *((int *)FLASH_HunderdPpm))/ ADC_struct.VAREF),1.6)) / (*((int *)FLASH_HunderdPpm) - *((int *)FLASH_ZeroPpm));
//					}
//					PpmCount.Mode = _exe;
//				}
				ADC_struct.Step++;
      ...
      ADC->CR1 &= ~(ADC_CR1_ADEN_Msk);
		...
		case 7:
			...
			break;
		case 8:
			if(ADC_struct.BatState == 0 && ADC_struct.HppmState == 0)
			{
//				if(*((uint32_t *)FLASH_Count) == 0xFFFFFFFF)
//				{
//					Display.Lppm = 0;
//				}
        Display.Ppm = ADC_struct.Ppm[2];
				Display.Mode = _exe;
				ADC_struct.Step = 0;
