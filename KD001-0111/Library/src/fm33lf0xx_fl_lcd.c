...
#define         IS_FL_LCD_LMUX(__VALUE__)               (((__VALUE__) == (FL_LCD_COM_NUM_4COM))||\
                                                        ...
                                                        ((__VALUE__) == (FL_LCD_COM_NUM_1COM))||\
                                                        ((__VALUE__) == (FL_LCD_COM_NUM_2COM))||\
                                                        ((__VALUE__) == (FL_LCD_COM_NUM_3COM))||\
...
static uint32_t FL_LCD_DisplayFreq(uint32_t freq)
{
    ...
    if((freq > 0) && (freq <= 100))
    {
        ...
        {
            case FL_LCD_COM_NUM_1COM:
                displayFreq = (uint32_t)(32768.0 / (float)(1 * freq * 2) + 0.5);
                break;
            case FL_LCD_COM_NUM_2COM:
                displayFreq = (uint32_t)(32768.0 / (float)(2 * freq * 2) + 0.5);
                break;
            case FL_LCD_COM_NUM_3COM:
                displayFreq = (uint32_t)(32768.0 / (float)(3 * freq * 2) + 0.5);
                break;            
...
static uint32_t FL_LCD_FlickTime(uint32_t time)
{
    ...
    {
        case FL_LCD_COM_NUM_1COM:
            stepTime = (float)(1 * FL_LCD_ReadDisplayFrequency(LCD) * 2 * 16 * 1000) / 32768.0;
            break;
        case FL_LCD_COM_NUM_2COM:
            stepTime = (float)(2 * FL_LCD_ReadDisplayFrequency(LCD) * 2 * 16 * 1000) / 32768.0;
            break;
        case FL_LCD_COM_NUM_3COM:
            stepTime = (float)(3 * FL_LCD_ReadDisplayFrequency(LCD) * 2 * 16 * 1000) / 32768.0;
            break;        
        case FL_LCD_COM_NUM_4COM:
...
FL_ErrorStatus FL_LCD_DeInit(LCD_Type *LCDx)
{
    ...
    /* 关闭外设总线始时钟和工作时钟 */
    FL_CMU_DisableGroup1BusClock(FL_CMU_GROUP1_BUSCLK_LCD);
    /* 锁定外设复位 */
...
FL_ErrorStatus FL_LCD_Init(LCD_Type *LCDx, FL_LCD_InitTypeDef *initStruct)
{
    ...
    /* 外设总线始时钟 */
    FL_CMU_EnableGroup1BusClock(FL_CMU_GROUP1_BUSCLK_LCD);
    /* 电流源电流控制 */
...
/**
  ...
  * @param  seg 待显示字段所在SEG。范围0-29。28 对应SEG30， 29对应SEG31
  * @param  state 待显示字段状态。0，熄灭；否则，点亮。
  *
  * @retval None
  */
